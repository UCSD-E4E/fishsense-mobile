...
class DBHelper {
    ...
    func createTable() {
        let query = "CREATE TABLE IF NOT EXISTS FishData(id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp INTEGER, length INTEGER, headx INTEGER, heady INTEGER, tailx INTEGER, taily INTEGER, lat DOUBLE, long DOUBLE, rgb TEXT, depth TEXT, mask TEXT);"
    ...
    func insert(timestamp: Int64, length: Int64, headx: Int64, heady: Int64, tailx: Int64, taily: Int64, lat: Double, long: Double, rgb: String, depth: String, mask: String) {
        let query = "INSERT INTO FishData(timestamp, length, headx, heady, tailx, taily, lat, long, rgb, depth, mask) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        var statement: OpaquePointer? = nil

        if sqlite3_prepare_v2(db, query, -1, &statement, nil) == SQLITE_OK {
            sqlite3_bind_int64(statement, 1, timestamp)
            sqlite3_bind_int64(statement, 2, length)
    ...
    func read(avg: Int) -> [FishData] {
        ...
        if sqlite3_prepare_v2(db, query, -1, &statement, nil) == SQLITE_OK {
            while sqlite3_step(statement) == SQLITE_ROW {
                ...
                let length = Int64(sqlite3_column_int(statement, 2))
                ...
                let model = FishData(id: id, timestamp: timestamp, length: length, headx: headx, heady: heady, tailx: tailx, taily: taily, lat: lat, long: long, rgb: rgb, depth: depth, mask: mask)
