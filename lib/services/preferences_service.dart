...
class PreferencesService extends ChangeNotifier {
  ...
  /// True once [load] has finished reading from disk. Widgets can render
  /// against [units] before this flips — the default (metric) matches the
  /// pre-existing display behavior, so a brief flash on cold start is a
  /// no-op.
  ...
  Future<void> load() async {
    ...
    _units = raw == 'imperial' ? UnitsSystem.imperial : UnitsSystem.metric;
    _loaded = true;
    notifyListeners();
  }
